...
#define Gmax 1000000
#define error 0.1  //le ponemos 4 ceros y genera mas rapido
...
int main()
{
	...
	for (x=0;x<mu;x++)
	{
		...
		for(y=0;y<num_var;y++)
		{
		    //matr_Pad[x][y].x = 10;//genGauss(0.0,1.0); //se evalua la funcion
			//matr_Pad[x][y].sigma = 10;//aleatorio(10.0,200.0);
			if((y==0) || (y==1))
			{
			...
			else if ((y==2) || (y==3))
			{
				...
				matr_Pad[x][y].x = datosP; //se evalua la funcion
				//matr_Pad[x][y].sigma = aleatorio(1.0,90.0)*0.0625;
				matr_Pad[x][y].sigma = -10;//genGauss(1.0,90.0)*0.0625;
			}
	...
	while((t<Gmax) && (aprox>error))
	{
		...
		for(s=0;s<landa;s++)
		{
			...
			for(i=0;i<num_var;i++)
			{
				...
				//recombinacionIntermedia
				 x=(matr_Pad[num_ale[0]][i].x+matr_Pad[num_ale[1]][i].x)/2;//x = matr_Pad[0][i].x;//matr_Pad[num_ale[1]][i].x;
				 sigma =(matr_Pad[num_ale[1]][i].sigma+matr_Pad[num_ale[0]][i].sigma)/2;//matr_Pad[0][i].sigma;//matr_Pad[num_ale[1]][i].sigma;
		...
		//print(matr_Pad, evaluciones, mu, num_var);
		t++;
        //aprox=fabs(5885.403282809389-evaluciones[0]);
	}
	printf("\n");
	print(matr_Pad, evaluciones, mu, num_var);
	printf("\n--- Iteracion Numero %d ---\n",t+1);
	int y;
	for (y=0;y<num_var;y++){
		printf(" x%d:%f sigma:%f",y+1,matr_Pad[0][y].x,matr_Pad[0][y].sigma);
	}
	printf("\n f(x): %f  \n",evaluciones[0]);
