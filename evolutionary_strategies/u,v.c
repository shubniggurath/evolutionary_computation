...
int main()
{
	double **sigma=NULL, *padre=NULL, padreFunc, hijoFunc,num,aprox,*evaluciones=NULL,*eval_H=NULL;
	double **uv;
	...
	for (x=0;x<mu;x++)
	{
		...
		{	
			...
			//se puede poner 10 y tarda mas
			matr_Pad[x][y].sigma = 0.0;
		}
	...
	while((t<Gmax) && (aprox>error))
	{
		...
		{	
			...
			{		
				...
				d =((matr_Pad[num_ale[0]][i].sigma+matr_Pad[num_ale[1]][i].sigma)/2)*0.0817;
		...
		for(x=0;x<landa;x++)
		{
			...
			for(z=0;z<num_var;z++)
			{
				double v_a=hijos[x][z].x;
				double del=hijos[x][z].sigma;
				
				hijos[x][z].x=hijos[min][z].x;
				hijos[min][z].x=v_a;
				
				hijos[x][z].sigma=hijos[min][z].sigma;
				hijos[min][z].sigma=del;
